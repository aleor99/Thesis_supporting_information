...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
  HAL_UART_Receive_IT(&huart2, UART1_rxBuffer, 17);
  /* USER CODE END 2 */
...
/////////////////////////////
{
  ...
  int bytes[10];
  char ad7746_flag=OFF, flag_OK=OFF;
  int count=0;
  char string[64];
  uint32_t starttime;
...
{
    HAL_UART_Transmit(&huart1, UART1_rxBuffer, 17, 1000);
    HAL_UART_Receive_IT(&huart2, UART1_rxBuffer, 17);
}
/* USER CODE END 4 */
